  ...
  try {
    ...
    const data = await C_vendorLedger.findAll({
      attributes: [
        ...
        [Sequelize.literal("IFNULL(paymentLedger.amount, 0)"), "creditAmount"],
        [
          Sequelize.literal(`
        (
          SELECT
        ...
        (
          (
            ...
            WHERE
              ...
              AND cl2.companyId = ${companyId}
              AND (cl2.date < \`P_C_vendorLedger\`.\`date\` OR (cl2.date = \`P_C_vendorLedger\`.\`date\` AND cl2.id < \`P_C_vendorLedger\`.\`id\`))
          ) + IFNULL(paymentLedger.amount, 0) - IFNULL(invoiceLedger.totalMrp, 0)
        )
      `),
          "remainingBalance",
        ],
  ...
  try {
    ...
    const data = await vendorLedger.findAll({
      attributes: [
        ...
        "id",
        [
          Sequelize.literal("IFNULL(invoiceVendor.mainTotal, 0)"),
          "debitAmount",
        ],
        [Sequelize.literal("IFNULL(paymentVendor.amount, 0)"), "creditAmount"],
        [
          Sequelize.literal(`
        (
          SELECT
            IFNULL(SUM(IFNULL(paymentVendor.amount, 0)- IFNULL(invoiceVendor.mainTotal, 0)), 0)
          FROM
        ...
        (
          (
            ...
            WHERE
              ...
              AND cl2.companyId = ${companyId}
              AND (cl2.date < \`P_vendorLedger\`.\`date\` OR (cl2.date = \`P_vendorLedger\`.\`date\` AND cl2.id < \`P_vendorLedger\`.\`id\`))
          ) + IFNULL(paymentVendor.amount, 0) - IFNULL(invoiceVendor.mainTotal, 0)
        )
      `),
          "remainingBalance",
        ],
